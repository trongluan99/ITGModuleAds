package com.ads.control.ironsource;
...
public class AppIronSource {
        ...
        if (mIronSourceBannerLayout != null) {
            ...
            IronSource.addImpressionDataListener(new ImpressionDataListener() {
                ...
                public void onImpressionSuccess(ImpressionData impressionData) {
                    ITGLogEventManager.logEventWithIronSourceAds(activity, impressionData);
                }
            });
    ...
    public void loadSplashInterstitialPlacements(Activity activity, AdCallback adListener, String placement, int timeOut) {
        isTimeout = false;
    ...
    public void showInterstitial(Context context, AdCallback adListener) {
        ...
        IronSource.addImpressionDataListener(new ImpressionDataListener() {
            @Override
            public void onImpressionSuccess(ImpressionData impressionData) {
                ITGLogEventManager.logEventWithIronSourceAds(context, impressionData);
            }
        });
    ...
    public void showInterstitialForPlacements(Context context, String placement, AdCallback adListener) {
    ...
    public void loadAndShowInter(Activity activity, int timeOut, String placementId, AdCallback adListener) {
        try {
            if (dialog != null && dialog.isShowing())
                dialog.dismiss();
        ...
        if (timeOut > 0) {
            ...
            rdTimeout = new Runnable() {
                ...
                public void run() {
                    ...
                    isTimeout = true;
                    if (IronSource.isInterstitialReady()) {
                        loadAndShowInterstitial(activity, adListener, dialog, placementId);
                        return;
                    }
                    if (adListener != null) {
                        adListener.onAdClosed();
    ...
    private void loadAndShowInterstitial(Context context, AdCallback adListener, PrepareLoadingAdsDialog dialog, String placementId) {
        if (handlerTimeout != null && rdTimeout != null) { // cancel check timeout
            handlerTimeout.removeCallbacks(rdTimeout);
        }
        if (IronSource.isInterstitialReady()) {
